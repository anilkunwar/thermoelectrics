...
st.markdown("""
Enter the number of components (n) for the system (e.g., 3 for ternary, 4 for quaternary). Upload one or more JSON files with the naming convention `AaBbCc...Xn.json` (e.g., `Bi1Sb1Te2.json` for ternary, `Bi1Sb1Te2Se1.json` for quaternary), where `a`, `b`, `c`, ..., `n` are stoichiometric coefficients. Each file contains a list of dictionaries with `x` (temperature in K) and `y` (Seebeck coefficient in μV/K). The script extracts the chemical formula from the filename, creates a CSV with `Formula`, `temperature(K)`, and `seebeck_coefficient(μV/K)`, featurizes the data with mole fractions (e.g., A: a/(a+b+c+...+n)), and allows downloading both the initial and featurized CSVs.
**Date and Time**: 08:49 PM CEST, Thursday, July 31, 2025
""")
...
for uploaded_file in uploaded_files:
    try:
        ...
        formula = os.path.splitext(filename)[0]  # Remove .json extension
        
        try:
            # Validate formula with pymatgen
            comp = Composition(formula)
            # Check if it has exactly n_components elements
...
st.write("Number of elements present in the data:", len(present_elements))
...
st.write("Elements present in the data are colored; absent elements are gray. Hover to see electronegativity and thermoelectric weight.")
...
# Display and allow download of featurized DataFrame
...
st.download_button(
    ...
    file_name='featurized_thermoelectric_data.csv',
    mime='text/csv',
