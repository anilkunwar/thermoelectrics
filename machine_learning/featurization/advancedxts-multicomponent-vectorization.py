...
def featurize_materials(df, available_elements, csv_columns):
    ...
    for _, row in df.iterrows():
        try:
            ...
            feature_vector = {col: 0.0 for col in csv_columns if col not in ['S.N.', 'Formula', 'modformula', 'temperature(K)', 'seebeck_coefficient(μV/K)']}
...
tab1, tab2 = st.tabs(["Upload and Process Data", "Edit Data"])
...
with tab1:
    ...
    **Date and Time**: 07:00 AM CEST, Sunday, August 03, 2025
    ...
    n_components = st.number_input("Enter the number of components (n, e.g., 3 for ternary, 4 for quaternary)", min_value=2, max_value=10, value=3, step=1, key="n_components")

    # File uploader for pre-existing featurized CSV
    csv_file = st.file_uploader("Upload pre-existing featurized CSV (e.g., featurized_thermoelectric_data.csv)", type=["csv"], key="csv_upload")
    ...
    if csv_file:
        ...
        csv_columns = ['S.N.', 'Formula', 'modformula'] + all_elements + [
        ...
        st.warning("No CSV uploaded. Using default column structure with 96 columns (including S.N.).")

    # File uploader for JSON files
    uploaded_files = st.file_uploader(f"Upload JSON files for {n_components}-component system (e.g., {'Bi1Sb1Te2.json' if n_components == 3 else 'Bi1Sb1Te2Se1.json'})", type=["json"], accept_multiple_files=True, key="json_upload")
    ...
    # Add S.N. column
    max_sn = df_csv['S.N.'].max() if 'S.N.' in df_csv.columns and not df_csv.empty else 0
    df_combined.insert(0, 'S.N.', range(max_sn + 1, max_sn + 1 + len(df_combined)))
    ...
    # Display featurized DataFrame
    st.subheader("Featurized Data (Before Editing)")
    st.write("New data appended to the pre-existing CSV (if uploaded), with absent features set to 0.")
    st.dataframe(df_final)

with tab2:
    st.markdown("""
    **Edit Data Tab**  
    Edit the featurized DataFrame below. Add new rows by clicking the '+' button or duplicate rows to auto-increment S.N. Edit the `reference` column or other fields as needed. Save changes and download the updated CSV, which can be reused as the input CSV for further updates.
    """)
    ...
    if 'df_final' in locals():
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                "S.N.": st.column_config.NumberColumn(disabled=True),  # Read-only S.N.
                "Formula": st.column_config.TextColumn(),
                "modformula": st.column_config.TextColumn(),
                "temperature(K)": st.column_config.NumberColumn(),
                "seebeck_coefficient(μV/K)": st.column_config.NumberColumn(),
                "electrical_conductivity(S/m)": st.column_config.NumberColumn(),
                "thermal_conductivity(W/mK)": st.column_config.NumberColumn(),
                "power_factor(W/mK2)": st.column_config.NumberColumn(),
                "ZT": st.column_config.NumberColumn(),
                "reference": st.column_config.TextColumn(),
                "sum_elements": st.column_config.NumberColumn(disabled=True),
                **{el: st.column_config.NumberColumn() for el in all_elements}
            },
            key="data_editor"
        )

        # Update S.N. for new rows
        if len(edited_df) > len(df_final):
            max_sn = edited_df['S.N.'].max()
            new_rows = edited_df.tail(len(edited_df) - len(df_final)).copy()
        ...
        st.write("Edited DataFrame with updated S.N. and sum_elements:")
        st.dataframe(edited_df)

        # Download updated CSV
        st.download_button(
            label="Download Edited Featurized CSV",
            data=edited_df.to_csv(index=False).encode('utf-8'),
            file_name='featurized_thermoelectric_data_edited.csv',
            mime='text/csv',
            key='download_edited'
        )
    else:
        st.warning("No data available to edit. Please process JSON files in the 'Upload and Process Data' tab first.")

# Display interactive periodic table
st.subheader("Interactive Periodic Table")
st.write("Elements present in the data (including edited data) are colored; absent elements are gray. Hover to see electronegativity and thermoelectric weight.")
present_elements = count_elements(edited_df if 'edited_df' in locals() else df_final)
