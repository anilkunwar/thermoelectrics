...
st.markdown("""
This tool inspects `thermoelectric_universe.db`, performs NER to extract chemical formulas and classify materials, and generates visualizations across three tabs. Libraries are loaded only when needed to minimize memory usage.

**Date and Time**: 09:12 PM CEST, Wednesday, August 20, 2025
""")
...
# Create tabs
tab1, tab2, tab3 = st.tabs(["Data Inspection", "Analysis", "Visualization"])

# Tab 1: Data Inspection
with tab1:
    st.subheader("Database Contents (thermoelectric_universe.db)")
    st.markdown("This tab displays the contents of `thermoelectric_universe.db` without loading heavy NLP libraries.")
    st.sidebar.header("Inspection Setup")
    st.sidebar.markdown("""
    **Dependencies**:
    - `pip install streamlit pandas sqlite3 psutil`
    """)
    ...
    log_container1.text_area("Inspection Logs", "\n".join(st.session_state.log_buffer), height=200)

# Tab 2: Analysis
with tab2:
    st.subheader("NER and Material Classification")
    st.markdown("This tab performs NER and classifies materials as p-type, n-type, or neutral. Libraries (spaCy, SciBERT) are loaded only when analysis is triggered.")
    st.sidebar.header("Analysis Parameters")
    st.sidebar.markdown("""
    **Dependencies**:
    - `pip install spacy transformers torch pymatgen pandas scikit-learn fuzzywuzzy python-Levenshtein clean-text psutil`
    - `python -m spacy download en_core_web_sm`
    """)
    query = st.sidebar.text_input("Query (e.g., thermoelectric materials)", "thermoelectric materials", key="analysis_query")
    material_types = st.sidebar.multiselect("Material Types", ["p-type", "n-type", "neutral"], default=st.session_state.material_types, key="analysis_types")
    relevance_threshold = st.sidebar.slider("Minimum Relevance Score (%)", 0.0, 100.0, st.session_state.relevance_threshold, key="analysis_relevance")
    pmi_threshold = st.sidebar.slider("PMI Threshold", 0.0, 5.0, st.session_state.pmi_threshold, key="analysis_pmi")
    similarity_threshold = st.sidebar.slider("Term Similarity Threshold", 0.5, 1.0, st.session_state.similarity_threshold, key="analysis_similarity")
    strict_validation = st.sidebar.checkbox("Strict Formula Validation (pymatgen)", st.session_state.strict_validation, key="analysis_strict")
    analyze_button = st.sidebar.button("Inspect and Analyze Database", key="analyze_button")
        ...
        if not query.strip():
            ...
            with st.spinner("Analyzing thermoelectric_universe.db..."):
                try:
                    ...
                    st.error(f"Failed to load analysis libraries: {e}")
                    update_log(f"Failed to load analysis libraries: {e}")
                    st.stop()
                ...
                def load_spacy():
                    try:
                        ...
                        update_log(f"spaCy loading error: {e}")
                        st.stop()
                ...
                # Extract terms
                def extract_terms(texts, query, similarity_threshold, top_n):
                    try:
                        nlp, _ = load_spacy()
                        term_counts = Counter()
                ...
                def analyze_universe_db(db_file=UNIVERSE_DB_FILE, query="thermoelectric materials", material_types=["p-type", "n-type", "neutral"], pmi_threshold=1.0, similarity_threshold=0.7, relevance_threshold=30.0):
                    try:
                        ...
                        st.session_state.analysis_results = (papers, metadata, term_counts)
                        sqlite_status = save_to_sqlite(pd.DataFrame(papers), metadata)
                        st.info(sqlite_status)
                        del tokenizer, model, nlp, matcher
                        gc.collect()
                        update_log("Cleared analysis libraries from memory")
                ...
                if metadata:
                    ...
                    if st.session_state.analysis_results:
                        try:
                            ...
                            st.error(f"Failed to prepare thermoelectric_metadata.db for download: {e}")
                            update_log(f"Failed to prepare thermoelectric_metadata.db for download: {e}")
                ...
                log_container2.text_area("Analysis Logs", "\n".join(st.session_state.log_buffer), height=200)

# Tab 3: Visualization
with tab3:
    st.subheader("Visualizations")
    st.markdown("This tab generates a histogram, word cloud, and co-occurrence network based on analysis results. Visualization libraries are loaded only when needed.")
    st.sidebar.header("Visualization Parameters")
    st.sidebar.markdown("""
    **Dependencies**:
    - `pip install matplotlib wordcloud networkx`
    """)
    max_terms = st.sidebar.slider("Max Terms in Histogram/Word Cloud", 10, 50, st.session_state.max_terms, key="vis_max_terms")
    context_window = st.sidebar.slider("Context Window for Network (words)", 10, 100, st.session_state.context_window, key="vis_context_window")
    min_edge_weight = st.sidebar.slider("Minimum Edge Weight for Network", 1, 10, st.session_state.min_edge_weight, key="vis_min_edge")
    visualize_button = st.sidebar.button("Generate Visualizations", key="visualize_button")
    ...
    if visualize_button:
        if not st.session_state.analysis_results:
            st.warning("Please run analysis in the Analysis tab first.")
        else:
            try:
                ...
                st.error(f"Failed to load visualization libraries: {e}")
                update_log(f"Failed to load visualization libraries: {e}")
                st.stop()

            papers, metadata, term_counts = st.session_state.analysis_results
            universe_df = inspect_universe_db()

            # Generate histogram
            def generate_term_histogram(term_counts, max_terms):
                try:
                    if not term_counts:
            ...
            def generate_word_network(texts, context_window, min_edge_weight):
                try:
                    nlp, _ = load_spacy()
                    G = nx.Graph()
                    key_terms = set(THERMOELECTRIC_KEYWORDS + P_TYPE_KEYWORDS + N_TYPE_KEYWORDS + list(SYNONYM_MAPPING.keys()))
            ...
            with st.spinner("Generating visualizations..."):
                ...
                if term_counts:
                    ...
                    st.warning("No key terms detected. Run analysis with adjusted thresholds.")
                ...
                if not universe_df.empty:
                    ...
                    st.warning("No valid data for network generation. Check thermoelectric_universe.db.")
                
                del plt, WordCloud, nx
                gc.collect()
                update_log("Cleared visualization libraries from memory")
                
                log_container3 = st.empty()
                log_container3.text_area("Visualization Logs", "\n".join(st.session_state.log_buffer), height=200)
