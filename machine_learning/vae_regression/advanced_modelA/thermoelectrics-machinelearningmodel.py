...
with tab1:
    ...
    #parallel_color_scale = st.sidebar.selectbox("Color Scale for Parallel Coordinates", ['Viridis', 'Plasma', 'Inferno', 'Magma'], index=0)
    ...
    if not df.empty:
        ...
        # 2D Scatter (Seebeck Coefficient)
        fig = px.scatter(
            x=z_2d[:, 0], y=z_2d[:, 1], color=output_feature_cleaned, color_continuous_scale=color_scale.lower(),
        ...
        # 2D Scatter (Dominant Element)
        fig_elements = px.scatter(
            x=z_2d[:, 0], y=z_2d[:, 1], color=dominant_elements_filtered, color_discrete_sequence=px.colors.qualitative.Set1,
