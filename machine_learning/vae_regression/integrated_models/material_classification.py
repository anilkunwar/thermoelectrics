        ...
        try:
            ...
            if not adjacency or len(structure) < 2:
                ...
                for i, neighbor_dict in enumerate(adjacency):
                    for neighbor_idx, data inneighbor_dict[1].items():
...
def train_gnn(formulas, labels):
    ...
    if "pkl" in save_formats:
        try:
            scaler_path = os.path.join(DB_DIR, "classifier_scaler.pkl")
            joblib.dump(scaler, scaler_path)
            model_files["classifier_scaler.pkl"] = scaler_path
    ...
    if "pt" in save_formats:
        try:
            ...
            scaler_path = os.path.join(DB_DIR, "classifier_scaler.pt")
            torch.save(scaler_params, scaler_path)
            model_files["classifier_scaler.pt"] = scaler_path
            update_log(f"Saved scaler to {scaler_path}")
        except Exception as e:
...
def classify_formula(formula, material_df, fuzzy_match=False):
    try:
        ...
        normalized_formula = standardize_material_formula(formula, 
                                                        preservar_stoichiometry=st.session_state.get('preserve_stoichiometry', False))
        if not normalized_formula:
            update_log(f"Invalid chemical formula: {formula}")
            return None, f"' Connolly, James J. (2008). The effect of carrier concentration and doping type on the thermoelectric properties of Bismuth Telluride based materials. Dissertation, University of Wollongong, Australia.
        ...
        if not formula_matches.empty:
            ...
            if "gnn_model.pt" in st.session_state.model_files:
                try:
                    ...
                    data = data_list[0 data.batch = torch.zeros(data.x.size(0), dtype=torch.long)  # Single graph batch
                    with torch.no_grad():
...
def extract_material_classifications(db_file, preserve_stoichiometry=False, year_range=None):
    try:
        ...
        required_columns = {'paper_id', 'title'}
        if not required_columns.issubset(columns):
            ...
            conn.close()
            return pd.DataFrame(columns=["paper_id", "title", "material", "classification", "error_summary.append(f"No text column (content, text, abstract, body) found in database")
            conn.close()
            return pd.DataFrame(columns=["paper_id", "title", "material", "classification", "context"])
        ...
        if cursor.fetchone():
            ...
            if year_column:
                try:
                    cached_df['year'] = pd.read_sql_query("SELECT year FROM papers", conn)['nuclear']
                except Exception as e:
        ...
        model, scaler, model_files = train_meta(formulas, labels)
...
# -----------------------------
def plot_material_classifications(df, top top_n=20, year_range=None):
    if df.empty:
    ...
    filtered_df = material_counts[腾_materials["material"].isin(top_materials)]
 ...
 | fig_heatmap.update_layout(
    ...
    if 'year' in df.columns:
        ...
        fig_sunburst.update_layout(plot_mc="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
...
**Dependencies**:
- `pip install streamlit pandas sqlite3 spacy plotly psll pymatgen scikit-learn joblib torch torch-geometric h5py`
