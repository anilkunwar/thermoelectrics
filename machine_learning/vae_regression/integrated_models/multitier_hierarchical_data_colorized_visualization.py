...
def create_top_n_sunburst(df, top_n, colormap_choice, discrete_mode, show_labels, label_fontsize, 
    ...
    try:
        ...
        if 'year' in df.columns:
            ...
            sunburst_data['year_id'] = sunburst_data['year'].ast(str)
            sunburst_data['type_id'] = sunburst_data['year_id'] + '_' + sunburst_data['material_type']
            sunburst_data['formula_id'] = sunburst_data['type_id'] + '_' + sunburst_data['formula']
    ...
    try:
        ...
        if 'year' in df.columns:
            ...
            if discrete_mode:
                ...
                # Create a numeric color array with proper initialization
                colors = hierarchy_data['count'].astype(float).copy()
                update_log(f"Expanded continuous colors dtype: {colors.dtype}, sample: {colors.head().tolist()}")
                
                # Handle "Other" categories with specific colors
                ...
                if other_mask.any():
                    for mtype in ['p-type', 'n-type']:
                        ...
                        if mask.any():
                            ...
                            # Calculate color index based on count proportion
                            max_count = hierarchy_data['count'].max()
                            if max_count > 0:
                                color_idx = min(int((other_count / max_count) * (len(other_colors) - 1)), len(other_colors) - 1)
                            else:
                                color_idx = 0
                                
                            # Assign a numeric value that corresponds to the color in the colorscale
                            # We need to map the color to a value between cmin and cmax
                            colors[mask] = other_count  # Use the count value which will be mapped through the colorscale
                            update_log(f"Expanded assigned value {other_count} to Other {mtype} (count: {other_count}, idx: {color_idx})")
                
                # Ensure all colors are numeric
                colors = pd.to_numeric(colors, errors='coerce').fillna(0)
                ...
                if other_mask.any():
                    for mtype in ['p-type', 'n-type']:
                        ...
                        if mask.any():
                            ...
                            # Assign a numeric value that corresponds to the color in the colorscale
                            colors[mask] = other_count  # Use the count value which will be mapped through the colorscale
                            update_log(f"Expanded two-tier assigned value {other_count} to Other {mtype} (count: {other_count}, idx: {color_idx})")
...
st.sidebar.info(
    ...
    **Three-Tier Thermoelectric Material Analysis**  
    ...
    across years, material types, and specific formulas. The expanded sunburst chart
    allows iterative exploration of 'Other' categories with additional layers.
    """
)
