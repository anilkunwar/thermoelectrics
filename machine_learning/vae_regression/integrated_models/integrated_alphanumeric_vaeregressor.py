...
if 'classifier_type' not in st.session_state:
    st.session_state.classifier_type = 'ANN'
...
if 'gnn_classifier' not in st.session_state:
    st.session_state.gnn_classifier = None
...
def classify_formula_gnn(elements, composition_dict, gnn_model):
    try:
        if not PYTORCH_GEOMETRIC_AVAILABLE:
            logger.error("PyTorch Geometric not available for GNN classification")
            raise ImportError("PyTorch Geometric is required for GNN classification")
        gnn_model.eval()
        with torch.no_grad():
...
try:
    ...
    # Load GNN classifier if available
    if PYTORCH_GEOMETRIC_AVAILABLE:
        ...
        st.session_state.gnn_classifier = gnn_classifier
    else:
        st.session_state.gnn_classifier = None
...
**Date and Time**: 05:56 AM CEST, Monday, September 01, 2025
""")
...
if st.session_state.use_manual_material_type:
    ...
    classifier_options = ['ANN']
    if PYTORCH_GEOMETRIC_AVAILABLE and st.session_state.gnn_classifier is not None:
        classifier_options.append('GNN')
    else:
        st.warning("GNN classifier is not available (PyTorch Geometric not installed or gnn_model.pt missing). Defaulting to ANN.")
    st.session_state.classifier_type = st.selectbox(
        "Select Classifier for Material Type Prediction",
        options=classifier_options,
        index=classifier_options.index(st.session_state.classifier_type) if st.session_state.classifier_type in classifier_options else 0,
        key='classifier_selector'
    )
    st.write(f"Using {st.session_state.classifier_type}-based material type prediction.")
    if st.session_state.selected_elements and sum(st.session_state.compositions.values()) > 0:
        try:
            ...
            if st.session_state.classifier_type == 'GNN':
                if not PYTORCH_GEOMETRIC_AVAILABLE or st.session_state.gnn_classifier is None:
                    raise ValueError("GNN classifier not available")
                material_type, summary_dict, _ = classify_formula_gnn(elements, comp_dict, st.session_state.gnn_classifier)
            else:  # ANN
                if st.session_state.ann_classifier is None or st.session_state.ann_scaler is None:
                    raise ValueError("ANN classifier or scaler not loaded")
            ...
            st.write(f"**Total Samples Processed**: {summary_dict.get('total_samples', summary_dict.get('total_graphs', 0))}")
            st.write(f"**Valid Predictions**: {summary_dict.get('valid_predictions', 0)}")
        except Exception as e:
            st.error(f"Failed to get {st.session_state.classifier_type} prediction: {e}. Defaulting to Neutral.")
            logger.error(f"{st.session_state.classifier_type} material type error: {e}")
            st.session_state.error_summary.append(f"{st.session_state.classifier_type} prediction error: {e}")
            st.session_state.sign_bias = 'Neutral'
    else:
        st.warning(f"Please select elements and normalize proportions to get a {st.session_state.classifier_type} prediction.")
        st.session_state.sign_bias = 'Neutral'
...
if st.button("Generate Ternary Diagram"):
    if len(st.session_state.selected_elements) > 0:
        ...
        if total == 0:
            ...
            st.write(f"**Material Type Used**: {st.session_state.sign_bias} ({'Manual' if st.session_state.use_manual_material_type else f'{st.session_state.classifier_type} Prediction'})")
