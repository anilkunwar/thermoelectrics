...
# Load material classification data
...
# Initialize material classifications
if 'material_classifications' not in st.session_state:
    st.session_state.material_classifications = extract_material_classifications(db_path)
    logger.info(f"Loaded material classifications with {len(st.session_state.material_classifications)} entries")
...
if st.session_state.use_manual_material_type:
    ...
    st.write("Using GNN-based material type prediction from thermoelectric database.")
    if st.session_state.selected_elements and sum(st.session_state.compositions.values()) > 0:
        try:
            # Construct chemical formula from compositions
                ...
                if error:
                    ...
                    if similar_formula:
                        ...
                        if st.button(f"Classify Suggested Formula: {similar_formula}", key="classify_similar"):
                            result, error, _ = classify_formula(similar_formula, st.session_state.material_classifications, fuzzy_match=True)
                            if error:
                                st.error(error)
                                st.session_state.sign_bias = 'Neutral'
                            else:
                                st.session_state.sign_bias = result['classification']
                                st.write(f"**Predicted Material Type**: {result['classification']} (Confidence: {result['confidence']:.2%})")
                                if result['count'] > 0:
            ...
            st.error(f"Failed to predict material type: {e}. Defaulting to Neutral.")
            logger.error(f"GNN classification error: {e}")
            st.session_state.sign_bias = 'Neutral'
    else:
        st.warning("Please select elements and normalize proportions to get a GNN-based prediction.")
        st.session_state.sign_bias = 'Neutral'
...
if st.button("Generate Ternary Diagram"):
    if len(st.session_state.selected_elements) > 0:
        ...
        if total == 0:
            ...
            st.write(f"**Material Type Used**: {st.session_state.sign_bias} ({'Manual' if st.session_state.use_manual_material_type else 'GNN Prediction'})")
            ...
            st.text_area("Logs", "\n".join(st.session_state.log_buffer), height=150, key="app_logs")
    else:
