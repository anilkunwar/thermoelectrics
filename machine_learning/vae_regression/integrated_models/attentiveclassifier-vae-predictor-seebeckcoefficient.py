...
class MaterialClassifier(nn.Module):
    def __init__(self, input_dim=66):
        ...
        self.model = nn.Sequential(
            nn.Linear(input_dim, 32), nn.ReLU(), nn.BatchNorm1d(32, momentum=0.05), nn.Dropout(0.3),
            nn.Linear(32, 16), nn.ReLU(), nn.BatchNorm1d(16, momentum=0.05), nn.Dropout(0.3),
...
def predict_material_type(composition_dict, temperature, available_elements, _scaler, _classifier):
    try:
...
try:
    ...
    classifier = MaterialClassifier(input_dim=66).to(device)
...
**Date and Time**: 06:38 PM CEST, Friday, August 22, 2025
""")
...
if st.button("Generate Ternary Diagram"):
    if len(st.session_state.selected_elements) > 0:
        ...
        if total == 0 and st.session_state.optimization_mode == "Manual":
            ...
            if literature_compositions:
                st.write("**Literature Compositions**:")
