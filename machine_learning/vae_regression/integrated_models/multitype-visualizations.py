...
def create_highlighted_sunburst(df, highlight_materials, colormap_choice, discrete_mode, show_labels, label_fontsize, 
    ...
    try:
        ...
        # Create custom text for highlighted materials
        ...
        # Enhanced text for highlighted materials
    ...
    try:
        ...
        fig = go.Figure(go.Sunburst(
            ...
            marker=dict(
                ...
                line=dict(width=[0] * len(hierarchy_data), color=['#FFFFFF'] * len(hierarchy_data))
            ),
            hovertemplate='<b>%{label}</b><br>Count: %{value}<br>Percentage: %{percentParent:.2%}<extra></extra>',
            textinfo="label+text" if show_labels else "none",
            texttemplate=(
...
def create_radar_chart(df, top_n, material_type=None, year_range=None):
    try:
        if df is None or df.empty:
...
def create_histogram(df, top_n, material_type=None, year_range=None):
    try:
        if df is None or df.empty:
...
if st.session_state.data_df is not None:
    ...
    if st.sidebar.button("Generate Radar Charts"):
        with st.spinner("Generating radar charts..."):
            with col1:
                st.write("All Materials")
                radar_all = create_radar_chart(st.session_state.data_df, top_n, year_range=year_range)
                if radar_all:
            ...
            with col2:
                st.write("p-type Materials")
                radar_p = create_radar_chart(st.session_state.data_df, top_n, material_type='p-type', year_range=year_range)
                if radar_p:
            ...
            with col3:
                st.write("n-type Materials")
                radar_n = create_radar_chart(st.session_state.data_df, top_n, material_type='n-type', year_range=year_range)
                if radar_n:
    ...
    if st.sidebar.button("Generate Histograms"):
        with st.spinner("Generating histograms..."):
            with col1:
                st.write("All Materials")
                hist_all = create_histogram(st.session_state.data_df, top_n, year_range=year_range)
                if hist_all:
            ...
            with col2:
                st.write("p-type Materials")
                hist_p = create_histogram(st.session_state.data_df, top_n, material_type='p-type', year_range=year_range)
                if hist_p:
            ...
            with col3:
                st.write("n-type Materials")
                hist_n = create_histogram(st.session_state.data_df, top_n, material_type='n-type', year_range=year_range)
                if hist_n:
    ...
    with st.expander("Publication Quality Tips"):
        ...
        **For publication-quality figures:**
        ...
        9. Export data as **CSV** or **JSON** for further analysis in other tools.
